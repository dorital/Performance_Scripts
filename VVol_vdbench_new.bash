...
#
# This script should be run on csopx and it will communicate with perf-proxy and perf-util
#
...
index=0
messages_vm_ip=`ssh perf-proxy ssh perf-util "cat /var/log/messages|grep "DHCPOFFER.*00:50:56:b1"|wc -l"`
if [[ $messages_vm_ip -ge $number_of_vm ]];then
	for vm_ip in `ssh perf-proxy ssh perf-util cat /var/log/messages|grep "DHCPOFFER.*00:50:56:b1"|tail -n $number_of_vm|awk '{print $8}'`;do
		echo $vm_ip
		array_vm_ip[$index]=$vm_ip
		if [[ `ssh perf-proxy ssh perf-util cat /root/.ssh/known_hosts|grep -w $vm_ip` ]];then
			:
		else
			ssh perf-proxy "ssh perf-util 'echo $vm_ip ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBP9is4DJkHL2FCQcYKw0MBQWetJIHM4bVj6DrmMv+mshtAdyEs2o1KpYFMyP95+1AGMp+/CmiG8swBUNqkAVUtY= >> /root/.ssh/known_hosts'"
		fi
		((index++))
	done
else
	old_messages=`ssh perf-proxy ssh perf-util ls -l /var/log/messages*|tail -1|awk '{print $9}'`
	old_messages_vm_ip=`echo $(($number_of_vm-$messages_vm_ip))`
	for vm_ip in `ssh perf-proxy ssh perf-util cat /var/log/messages|grep "DHCPOFFER.*00:50:56:b1"|tail -n $messages_vm_ip|awk '{print $8}'`;do
		echo $vm_ip
		array_vm_ip[$index]=$vm_ip
		if [[ `ssh perf-proxy ssh perf-util cat /root/.ssh/known_hosts|grep -w $vm_ip` ]];then
			:
	...
	for vm_ip in `ssh perf-proxy ssh perf-util cat $old_messages|grep "DHCPOFFER.*00:50:56:b1"|tail -n $old_messages_vm_ip|awk '{print $8}'`;do
		echo $vm_ip
		array_vm_ip[$index]=$vm_ip
		if [[ `ssh perf-proxy ssh perf-util cat /root/.ssh/known_hosts|grep -w $vm_ip` ]];then
			:
