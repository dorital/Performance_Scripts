		...
		if ping.wait() !=0:
			...
			if ssh.wait() !=0:
				...
				if host_os == 'linux':
					...
					host_list_ok[host] = {'OS':host_os,'version':os_version,'java version':java_version,'fc_speed':fc_speed,'total_fc_ports':total_ports,'total_fc_active':total_active,'host_bw_MB/s':host_bw,'total_cpu':total_cpu,'total_memory':total_memory}
				elif host_os == 'esx':
					print "host %s is esxi" %(host)
					host_list_ok[host] = 'esxi'					
	# need to edit print to look nice
	if sort == 'name':
		print "list of free hosts sorted by %s:" %(sort)
		for host_key in sorted(host_list_ok):
			print host_key,host_list_ok.get(host_key)
	else: 
		sort_dir={}
		print "list of free hosts sorted by %s:" %(sort)
