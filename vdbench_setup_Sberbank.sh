...
hostlist=(F657-07-659-05 F657-07-659-06 F657-07-659-09 F657-07-659-10 F644-01-742-01 F644-01-742-02 F644-01-742-03 b5-05-707-05 b5-05-707-06 b5-05-707-07 b5-05-707-08 b5-05-707-09)
...
echo -e "to_create_vdbecnh:\t"$to_create_vdbench
...
###	vdbench_sd_output="${system_name}_${interface}_${active_dataset}GB_vdbench_config"
	vdbench_vol_size="1500"
	vdbench_sd_output="dori_Sberbank_vdbench.cfg"
	rm -f /root/vdbench/$vdbench_sd_output
	echo "compratio=2.86" > /root/vdbench/dori/$vdbench_sd_output
	echo "#dedupratio=2,dedupunit=8k,dedupsets=5%" >> /root/vdbench/dori/$vdbench_sd_output
	echo "debug=25" >> /root/vdbench/dori/$vdbench_sd_output
	echo "hd=default,vdbench=/root/vdbench/bin,user=root,shell=ssh" >> /root/vdbench/dori/$vdbench_sd_output
	for host in "${hostlist[@]}";do 
		hd=$( indexof $host )
		echo "hd=hd$hd,system=$host" >> /root/vdbench/dori/$vdbench_sd_output
	done
	#
...
###		ssh $host "multipath" > /dev/null
	...
	# for each volume checkon the host the attached mpath using the wwn
		for dm in `ssh $host multipath -ll |grep -i $system_serial_hex |cut -f2 -d"-"|cut -f1 -d" "|sort -n`;do  
			echo "sd=sd$sd,host=hd$hd,lun=/dev/dm-$dm,openflags=(o_direct,o_sync,fsync),size=$vdbench_vol_size""g,threads=10,hitarea=170m" >> /root/vdbench/dori/$vdbench_sd_output
			((sd++))
		done
	done
	echo "rd=default,xfersize=8k,iorate=max,elapsed=10m,interval=10,openflags=o_direct,hitarea=400m,pause=1m" >> /root/vdbench/dori/$vdbench_sd_output
	echo "#rd=prealloc,sd=*,elapsed=1000h,interval=10,rdpct=0,openflags=o_direct,iorate=max,xfersize=256K,seekpct=0,maxdata=1,threads=1,pause=20m" >> /root/vdbench/dori/$vdbench_sd_output
	echo "#rd=warmup,sd=*,elapsed=20m,interval=10,openflags=o_direct,iorate=max,xfersize=1m,rhpct=100,whpct=100" >> /root/vdbench/dori/$vdbench_sd_output
	echo "#rd=Read_Miss_8k_FFR,sd=*,elapsed=30m,iorate=300000,xfersize=8k,rdpct=100,rhpct=0" >> /root/vdbench/dori/$vdbench_sd_output
	echo "#rd=seq_read,sd=*,elapsed=1000h,interval=10,openflags=o_direct,iorate=max,xfersize=256k,rdpct=100,seekpct=0,maxdata=1,threads=1,pause=20m" >> /root/vdbench/dori/$vdbench_sd_output
	echo "#rd=Read_miss_8k_noFFR,sd=*,elapsed=30m,iorate=300000,xfersize=8k,rdpct=100,rhpct=0" >> /root/vdbench/dori/$vdbench_sd_output
else
	echo "[INFO] Not running vdbench config creation"
fi
echo "[INFO} vdbench config file was created: /root/vdbench/dori/$vdbench_sd_output"
exit
