...
is_it_iSCSI="no"
is_it_nvme="no"
zone_model="F" #F-Full, H-Half, S-Single
sia_user="ibmsc"
sia_user_password="P@ssw0rd"
system_name="gen4d-pod-331"
active_dataset=15000
test_name="perf_vvol"
pe_number=2
#hostlist=(tile1 tile2 tile3 tile4 tile5 tile6 tile7 tile8 tile9 tile10 tile11 tile12 tile13 tile14 tile15 tile16 tile17 tile18 tile19 tile20)
#hostlist=(tile12 tile13 tile14 tile15 tile16 tile17 tile18 tile19 tile20)
...
domain_size=`echo "$active_dataset*12"|bc`
domain_name=${test_name}_domain
ssh $system_name xcli.py domain_create domain=$domain_name max_pools=20 max_volumes=32000 size=$domain_size max_cgs=20 -y 1>/dev/null
...
if [ $to_do_fccon == "yes" ];then
	...
	echo "[info] Current zoning for system $system_name"
	ssh stcon /opt/FCCon/fcconnect.pl -op ShowActZones|grep -i "${system_name}"|grep -i "tile"
fi
#
