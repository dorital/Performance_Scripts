...
#		host_fc_wwpn=`ssh $host cat /sys/class/fc_host/host*/port_name|cut -f2 -d "x"`
		...
		# Adding hosts wwpn on A9K system
			echo ssh $system_name xcli.py host_add_port host=$host fcaddress=$fc_wwpn
			ssh $system_name xcli.py host_add_port host=$host fcaddress=$fc_wwpn
			echo "ssh stcon '/opt/FCCon/fcconnect.pl -op CreateZone -zone Z_${system_name}-${host}_${zone} -wwn $fc_wwpn $stcon_storage'"
